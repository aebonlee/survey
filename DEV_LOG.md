...
## 2025-04-09 — 연구방법 페이지 리스트 데이터를 테이블(table)로 변환

### 개요

연구방법 관련 4개 페이지(연구방법, 양적연구방법, 질적연구방법, AHP)에서 `<ul>/<li>` 형태의 구조적 비교 데이터를 `<table className="guide-table">` 형태로 변환하여 가독성 대폭 개선.

### CSS 추가

| 파일 | 변경 |
|------|------|
| `src/styles/guide-pages.css` | `.guide-table`, `.table-responsive` 스타일 추가 (헤더 배경, 행 hover, 다크모드 지원) |

### 변환된 테이블 목록

#### `Methods.tsx` (4개 테이블 변환)

| 섹션 | 변환 내용 |
|------|-----------|
| 연구 패러다임 | 실증주의/해석주의/실용주의 → 패러다임·핵심 가정·연구 접근 3열 테이블 |
| 양적 vs 질적 비교 | 목적/자료수집/분석/표본/설계/추구가치 → 6행 비교 테이블 |
| 변수 유형 | 독립/종속/매개/조절/통제 → 변수유형·역할·예시 3열 테이블 |
| 측정 수준 | 명목/서열/등간/비율 → 특성·가능한 통계·예시 4열 테이블 |

#### `QualitativeMethods.tsx` (4개 테이블 변환)

| 섹션 | 변환 내용 |
|------|-----------|
| 질적연구 핵심 특성 | 자연주의적 탐구 외 4개 → 특성·설명 2열 테이블 |
| 윤리적 고려사항 | 사전 동의 외 4개 → 윤리원칙·설명 2열 테이블 |
| 목적적 표집 유형 | 5개 표집 유형 → 표집유형·설명 2열 테이블 |
| 삼각검증 유형 | 4개 삼각검증 유형 → 유형·설명 2열 테이블 |

#### `Ahp.tsx` (6개 테이블 변환)

| 섹션 | 변환 내용 |
|------|-----------|
| AHP 3대 원리 | 분해/비교판단/종합 → 원리·설명 2열 테이블 |
| 적용 분야 | 경영학/공학/공공정책/의료/교육 → 분야·적용 예시 2열 테이블 |
| 계층구조 설계 원칙 | 4개 원칙 → 설계원칙·설명 2열 테이블 |
| 계층구조 설계 흔한 실수 | 5개 실수 → 흔한 실수·해결 방안 2열 테이블 |
| AHP 논문 구조 | 5개 섹션 → 구성·주요 내용 2열 테이블 |
| 심사 시 흔한 지적사항 | 5개 지적사항 → 심사 지적·대응 전략 2열 테이블 |

### 커밋 이력

| 해시 | 메시지 |
|------|--------|
| (현재) | feat: 연구방법 페이지 리스트 데이터를 guide-table로 변환 |
